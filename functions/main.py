...
def load_region_selection_mapping():
...
async def get_provinces(region: str):
    ...
    return sorted(mapping.get(region, {}).keys())
...
async def get_municipalities(region: str, province: str):

    mapping = load_region_selection_mapping()

    return sorted(mapping.get(region, {}).get(province, []))
